...
ВАЖНЫЕ ПРАВИЛА:
- КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать Markdown-разметку любого вида: звёздочки (*), подчёркивания (_), решётки (#), обратные кавычки (`) и т.п.
- КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать хэштеги (слова с #).
- Для выделения важного используй ТОЛЬКО HTML-теги: <b>жирный</b>, <i>курсив</i>, <u>подчёркнутый</u>. Но делай это умеренно, только когда действительно нужно.
- Если не уверен в форматировании — пиши обычным текстом без тегов.
- Не добавляй никаких служебных символов, кроме эмодзи и знаков препинания."""
...
Не используй Markdown или хэштеги в своих ответах (они будут в JSON)."""
...
Не используй Markdown и хэштеги в своих ответах (ты даёшь чистый текст)."""
